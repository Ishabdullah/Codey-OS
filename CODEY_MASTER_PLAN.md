...
- [ ] **Concurrency test** — **Live pass RUN 2026-08-26; negative result
      ...
      **`NEW-432`**, surfaced during pre-flight (a stale resident-slot
      ...
      corrupting the ceiling until manually cleaned up). **Checkbox
      stays unchecked — confirmed-broken-on-real-hardware, not merely
      pending verification.** Needs its own scoped fix round on
      `NEW-430`/`NEW-431` before this can close.
