    ...
    if old_content:
        ...
        if diff:
            ...
            for line in diff[2:]:  # skip --- +++ headers
                if shown >= 60:
                    diff_text.append(
                        f"... ({len(diff) - 2 - shown} more diff lines)\n", style="dim"
                    )
                    break
                if line.startswith("+"):
...
def show_shell(command, output, error=False):
    ...
    console.print(Panel(content, title="Shell", border_style=border, box=box.ROUNDED))
...
def show_tool_generic(name, args, result):
    ...
    console.print(
        Panel(content, title=name.replace("_", " ").title(), border_style="dim", box=box.ROUNDED)
    )
...
def show_tdd_failure(test_name, error_text):
    ...
    console.print(Panel(content, title="Test Failure", border_style="red", box=box.ROUNDED))
    ...
    if passed == total:
        ...
        console.print(
            Panel(
                ...
                title="[yellow]Partial Pass[/yellow]",
                border_style="yellow",
                box=box.ROUNDED,
            )
        )
...
def _build_task_panel(queue):
    ...
    from core.taskqueue import STATUS_DONE, STATUS_FAILED, STATUS_RUNNING
