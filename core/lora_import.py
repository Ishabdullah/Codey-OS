...
def merge_lora_with_llama_cpp(
    adapter_path: str, base_model_path: str, output_path: str, quantize: str = "q4_0"
) -> Tuple[bool, str]:
    """
    ...
    if not llama_dir.exists():
        return (
            False,
            "llama.cpp not found. Install with: git clone https://github.com/ggerganov/llama.cpp ~/llama.cpp",
        )
...
def _merge_with_python(
    adapter_dir: Path, base_model: Path, output: Path, quantize: str
) -> Tuple[bool, str]:
    ...
    try:
        # Apply LoRA
        llm.apply_lora_from_disk(str(adapter_dir), base_model=str(base_model))
    except Exception as e:
...
def swap_to_finetuned_model(model_path: str, model_variant: str = "primary") -> Tuple[bool, str]:
    """
...
def rollback_to_backup(backup_path: str, model_variant: str) -> Tuple[bool, str]:
    """
...
def import_lora_adapter(
    ...
    if merge_on_device:
        ...
        success, msg = merge_lora_with_llama_cpp(
            adapter_path, base_model, str(output_path), quantize
        )
