...
class Planner:
    """
    Internal task planner for Codey-OS.
    ...
    def fail_task(self, task_id: int, error_msg: str) -> bool:
        ...
        if task.retry_count < self.max_retries:
            ...
            self.state.execute("UPDATE task_queue SET status = 'pending' WHERE id = ?", (task_id,))
            warning(
                f"Planner: task {task_id} failed, retrying ({task.retry_count}/{self.max_retries})"
            )
        else:
