...
from core.display import (show_file_write, show_patch, show_shell,
                          show_tool_generic)
...
from tools.file_tools import (tool_append_file, tool_list_dir, tool_read_file,
                              tool_write_file)
    ...
    try:
        ...
        if (_is_write or _is_patch) and not result.startswith("[ERROR]"):
            ...
            if _lpath.endswith(".py"):
                try:
                    from core.linter import format_issues, run_linter
                    ...
                    if _issues:
                        ...
                        if _warnings:
                            ...
                            _lwarn(
                                f"[Linter/{_linter_used}] {len(_warnings)} style warning(s) in {_lpath}:"
                            )
                            for _w in _warnings[:5]:
                                _lwarn(f"  Line {_w.line}: [{_w.code}] {_w.message}")
                            if len(_warnings) > 5:
                                _lwarn(
                                    f"  ... and {len(_warnings) - 5} more (run /review for full list)"
                                )
                except Exception:
...
def is_hallucination(response, user_message, tools_used):
    ...
    needs_file = any(
        k in msg_lower for k in ["create", "write", "make", "build", "implement", "add"]
    )
...
def _auto_apply_peer_code(peer_output, context_message=""):
    ...
    if not files_written:
        ...
        if context_message:
            _fname_re = re.compile(r"\b([\w][\w\-]*\.(?:py|js|ts|html|css|json))\b")
...
def run_agent(
    ...
    if not _in_subtask:
        ...
        if _peer_name and _peer_task:
            ...
            if _peer_name in _by_name:
                ...
                _enriched_task = f"Task: {_peer_task}" + (
                ...
                if not _is_design_only:
                    ...
                    if _referenced_peer:
                        ...
                        if _prior_output:
                            info(
                                f"Injecting {_referenced_peer}'s previous output into {_peer_name}'s context"
                            )
                            _enriched_task = (
                                f"Task: {_peer_task}\n\n"
                                ...
                                f"{_prior_output}\n\n"
                                f"Your task: {_peer_task}" + _FORMAT_INSTRUCTIONS
                            )

                if _is_review:
                    # Data-privacy gate: warn before sending local file contents
                    # to an external AI service. This is explicit and opt-in.
                    from utils.logger import confirm as _priv_confirm
                    from utils.logger import warning as _priv_warn
                ...
                if _mgr.is_peer_error(_output):
                    ...
                    if _files_written:
                        ...
                        _suc(
                            f"[Peer: {_peer_name}] done. Applied {len(_files_written)} file(s): {', '.join(_files_written)}"
                        )
    ...
    from core.display import console, show_task_plan
    ...
    if is_qa:
        messages.append(
            {
                "role": "user",
                "content": "IMPORTANT: This is a question or conversation. Respond with plain text only. DO NOT use any tools. Keep your response concise — 2-3 sentences max unless more detail is needed.",
            }
        )
        ...
        if tool_dict:
            ...
            if is_qa and name not in ["read_file", "list_dir", "note_save", "note_forget"]:
                ...
                messages.append(
                    {
                        "role": "user",
                        "content": "Just answer my question directly with text. No tools needed. Final answer format: 'I can help with [tasks].'",
                    }
                )
                continue
            ...
            if name in ["write_file", "patch_file", "append_file"]:
                ...
                if path and path.lower() not in msg_low:
                    ...
                    if not _P(path).exists() and not any(
                        k in msg_low for k in ["create", "write", "new", "make"]
                    ):
                        ...
                        _missing_file_retry = (
                            auto_retries > 0 and "no such file" in last_tool_result.lower()
                        )
                        if not _missing_file_retry:
                            ...
                            messages.append(
                                {
                                    "role": "user",
                                    "content": f"I didn't ask to modify '{path}'. Please answer my question directly.",
                                }
                            )
                            continue
            ...
            if sig in tools_used:
                ...
                messages.append(
                    {
                        "role": "user",
                        "content": "Already ran that. Result: "
                        + last_tool_result[:200]
                ...
                if name == "shell" and "[CANCELLED]" in last_tool_result:
                    ...
                    if any(w in msg_low for w in _file_words):
                        messages.append(
                            {
                                "role": "user",
                                "content": "Command was not run. Use the write_file tool instead to create the file. Output ONLY a <tool> block with write_file.",
                            }
                        )
                        continue
            ...
            if name == "write_file" and not any(
                ...
                _is_fix = any(
                    k in user_message.lower()
                    for k in ["fix", "bug", "patch", "correct", "repair", "debug"]
                )
                if _is_fix:
                    from pathlib import Path as _Path

                    _test_candidates = list(_Path.cwd().glob("test_*.py")) + list(
                        _Path.cwd().glob("*_test.py")
                    )
                    if _test_candidates:
