# Codey-V3 Tools Embedding Pipeline — Design Plan
...
2. Converts raw examples into Codey-V3 tool-call format
...
The pipeline is designed to run on-device (Termux/Android) with lightweight models, or off-device for large dataset processing. It reuses Codey-V3's existing embedding infrastructure (nomic-embed-text on port 8082) wherever possible.

---

## 2. Codey-V3 Tool System — Key Findings
...
> **Note on naming:** The example in the task description uses `"arguments"` and `"run_termux_command"`. The canonical Codey-V3 format uses `"args"` and `"shell"`. This pipeline normalizes everything to the canonical format.
...
**Purpose:** Map normalized examples to one or more Codey-V3 tool calls.
...
**Recommendation:** Use **nomic-embed-text** via port 8082 for consistency with Codey-V3's existing RAG pipeline. Fall back to `all-MiniLM-L6-v2` if the embed server is offline.
...
| Command passes Codey-V3 metacharacter validation | +0.1 |
...
## 13. Integration with Codey-V3

Once the pipeline runs, the outputs plug directly into the existing Codey-V3 systems:
...
Codey-V3 tool calls, and how it feeds the embedding pipeline.  All datasets listed
...
that call functions with structured arguments. Directly analogous to the Codey-V3
...
**Transformation to Codey-V3 format:**
...
3. Map function name: generic API names (`get_weather`, `search_web`) → nearest
   Codey-V3 tool.  Direct-execution functions (`run_code`, `execute_command`) →
   `shell`.  File-output functions → `write_file`.
4. Discard examples where the function name has no plausible Codey-V3 equivalent.

**Name mapping table:**
| Glaive function pattern | Codey-V3 tool |
|------------------------|---------------|
...
The JSON schema format closely mirrors Codey-V3's `args` dict structure.
...
Codey-V3 `write_file` pattern: "write a Python function that does X."
...
with tests. This is the most realistic approximation of Codey-V3's actual coding
workflow.
...
for training Codey-V3 on "write a complete, working Python program" tasks.
...
For Codey-V3 the Python split is the highest priority; JS is secondary (Node.js
...
directly to Codey-V3. Use this as a **secondary source** — sample strategically by
...
is exactly the Codey-V3 agentic loop: `write_file` → `shell` → `patch_file`.
...
**Gap:** Most datasets have single-step responses. Codey-V3's orchestrator generates
