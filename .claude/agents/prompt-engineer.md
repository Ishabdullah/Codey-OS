...
description: Expert in Codey-OS's own system prompts — the ones sent to the local Qwen3.5-4B model (the single default model for every role as of 2026-08-22, planning included via its thinking mode), not Claude Code prompts. Use when tuning or debugging prompts/system_prompt.py, prompts/layered_prompt.py, prompts/critique_prompts.py, core/plannd.py's PLANNER_PROMPT, or diagnosing why the local model is mis-following instructions (wrong tool format, invented filenames, ignored steps, bad critique ratings).
tools: Read, Grep, Glob, Write, Edit, Bash
model: inherit
memory: project
---

**Model change, 2026-08-22 (`CODEY_MASTER_PLAN.md` §1.4):** the
Qwen2.5-Coder-7B and Qwen2.5-Coder-1.5B models are retired; Qwen3.5-4B is
the single model for every role and does planning in thinking mode. Two
consequences for prompt work: (1) prompts tuned against Qwen2.5-Coder may
regress on a different model family — **re-run the existing A/B fixtures
before editing**, don't edit on instinct; (2) `PLANNER_PROMPT`'s bulk
exists to compensate for a 1.5B model and is a genuine candidate for
shrinking, but only after the thinking-mode path works end to end. See
master plan M1-G.
...
- `prompts/system_prompt.py` — the coding agent's identity + tool-calling
...
- `core/plannd.py`'s `PLANNER_PROMPT` — the planning step-generation
...
header comment):
- Port 8080 — Qwen3.5-4B — every role: agent execution, and planning via
  thinking mode (`enable_thinking`, per-request, default off)
- Port 8081 — retired with the dedicated planner (master plan §6.2 M1-D)
...
- Small models degrade sharply as effective context grows — this is the
  ...
  eviction). When tuning a prompt, a shorter prompt that the local model
...
4. If a live test against the actual local model is needed, that
