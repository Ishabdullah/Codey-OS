...
│                              Ish to run himself (not run by Antigravity live)
...
10. Before defining a new subagent with `define_subagent`, check if an
    existing defined agent already fits the job. Only create a new one
    if none of the existing agents genuinely cover it — subagent sprawl
    makes the pipeline harder to reason about, not easier.
